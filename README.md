...
陈雨钦的前端组件库。把散在翻板墙、沐言书坊、Ripple、AI 互动故事、Yuqin portfolio、YoRHa-A2 这些项目里的组件、动效和三维场景收在一处 —— 能浏览、能查源码、能复制走。

从**翻页书**开始也收直接为库写的件,不再只是收编 —— 那类件的源码 import 是通的,不需要下面说的补 shim 那一套。

**公开。** 每一条都标了归属和上游来源,组件名旁边就是,不用点进去看。

自研和改造都算自己的作品 —— 起点是别处的灵感,代码是自己改出来的,那就是自己的。移植那五件不算,它们只是换了个地方放,标着 `移植` 和上游是谁。
...
**53 组全部有预览** —— 49 个演示台 + 4 个打包好的静态产物(翻板墙 / 卡片环形轮播 / 沐言书坊 / 书架轮播)。
...
|---|---|
| **自研** | 完全自己写的 |
| **改造** | 拿别处当起点,自己大幅改过:换 token、改挂载方式、加交互 |
| **移植** | 基本原样搬进来的 |
| **待查** | 来源没标注,还没核实 |

**自研 + 改造 = 我的**(46 件)。首页那格统计算的就是这两档。移植 5 件、待查 2 件不算。

`source` 字段单独记上游是谁,现在有 12 件标 `React Bits`、7 件标 `React Bits Pro`。`card-carousel-dom` 是看着 React Bits CircularGallery 的观感自己用 DOM 重写的,不是改它的代码,所以算自研、不标上游。
