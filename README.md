...
**55 组全部有预览** —— 51 个演示台 + 4 个打包好的静态产物(翻板墙 / 卡片环形轮播 / 沐言书坊 / 书架轮播)。
...
**自研 + 改造 = 我的**(48 件)。首页那格统计算的就是这两档。移植 5 件、待查 2 件不算。

`source` 字段单独记上游是谁,现在有 12 件标 `React Bits`、8 件标 `React Bits Pro`。`card-carousel-dom` 是看着 React Bits CircularGallery 的观感自己用 DOM 重写的,不是改它的代码,所以算自研、不标上游。
