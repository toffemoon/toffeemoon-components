...
export const MODES = {
  /** 纯随机:最接近视频里那种"乱"的观感 */
  随机: (col, row, cols, rows, seed) => hash(col, row, seed),

  /** 对角波浪:从左下角推到右上角,最容易看出"波"的方向 */
  对角波浪: (col, row, cols, rows) => {
    const span = cols - 1 + (rows - 1) || 1
    return (col + row) / span
  },

  /** 中心扩散:从正中间往四周炸开 */
  中心扩散: (col, row, cols, rows) => {
    const cx = (cols - 1) / 2
    const cy = (rows - 1) / 2
  ...
  /** 中心收拢:从四周往中间收 */
  中心收拢: (col, row, cols, rows) => 1 - MODES['中心扩散'](col, row, cols, rows),

  /** 列扫:一列一列翻,最规整 */
  列扫: (col, _row, cols) => col / (cols - 1 || 1),

  /** 行扫:一行一行翻 */
  行扫: (_col, row, _cols, rows) => row / (rows - 1 || 1),

  /** 分组批次:随机分成若干批,批内同时翻——比纯随机更有"节拍" */
  分组批次: (col, row, cols, rows, seed) => {
    const batches = 6
  ...
  /** 涟漪:从中心出发的多道同心波 */
  涟漪: (col, row, cols, rows) => {
    const d = MODES['中心扩散'](col, row, cols, rows)
