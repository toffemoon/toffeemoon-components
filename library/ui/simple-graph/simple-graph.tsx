...
export interface SimpleGraphProps {
  ...
  animateOnce?: boolean;
  /** Ripple 加:横向数值带(基线带),用内部同一套 y 标尺 */
  bands?: { from: number; to: number; color?: string; label?: string }[];
  /** Ripple 加:竖向标记线,按数据点下标定位 */
...
const SimpleGraph = ({
  ...
  const { points, pathD, toY } = useMemo(() => {
    if (!data || data.length === 0) {
      return { points: [], pathD: "", toY: () => 0 };
    }
    ...
    const calculatedPoints = data.map((d, i) => {
      ...
      return { x, y, value: d.value, label: d.label, dotColor: d.dotColor };
    });
    ...
    return {
      ...
      pathD: path,
      /* bands / markers 与折线共用同一套标尺 */
      toY: (v: number) =>
        graphPadding +
        graphHeight -
        ((v - paddedMin) / paddedRange) * graphHeight,
    };
  }, [data, curved]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Ripple 加:基线带(如 30 日均值 ±7% 的 usual range) */}
        {bands.map((band, i) => {
          const yTop = toY(band.to);
          const yBottom = toY(band.from);
          const color = band.color ?? "#37C2BA";
          return (
            <motion.g
              key={`band-${i}`}
              initial={{ opacity: 0 }}
        ...
        {/* Ripple 加:竖向标记线(如 CASE OPENED) */}
        {markers.map((marker, i) => {
          const pt = points[marker.index];
          if (!pt) return null;
          const color = marker.color ?? "#e6b45a";
          return (
            <motion.g
              key={`marker-${i}`}
              initial={{ opacity: 0 }}
              ...
              transition={{
                duration: 0.5,
                delay:
                  (marker.index / (points.length - 1 || 1)) *
                    animationDuration +
                  0.2,
              }}
            >
          ...
          points.map((point, index) => (
            ...
            >
              ...
              <motion.circle
                cx={point.x}
                cy={point.y}
                r={point.dotColor ? dotSize + 1.5 : dotSize}
                fill={point.dotColor ?? dotColor}
                stroke="#0a0c0f"
                strokeWidth="2"
                ...
                >
                  ...
                  >
                    <div className="relative">
                      <div className="whitespace-nowrap rounded-lg border border-line bg-panel px-3 py-2 text-ink shadow-lg">
                        {calculatePercentageDifference && hoveredIndex > 0 ? (
                          (() => {
                          ...
                          <div className="text-sm font-semibold">
                            {Number.isInteger(points[hoveredIndex].value)
                      ...
                      <div
                        className="absolute left-1/2 h-0 w-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent border-t-panel"
                        style={{
                          bottom: "-4px",
